  ...
  with tf.compat.v1.variable_scope(scope, 'MobilenetV1', [inputs]):
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding=padding):
      ...
      if partition_layer =='input':
          partition_layer_index=-1
      else:
          partition_layer_index = int(partition_layer.split("_")[1])

      if partition_layer_index ==13: # only the final laeyer computes on the edge
          return net,end_points


      for i, conv_def in enumerate(conv_defs):
        #print(i,conv_def)
        if i<=partition_layer_index:
            continue
  ...
  with tf.compat.v1.variable_scope(
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      net, end_points = mobilenet_v1_base(inputs, scope=scope,
                                          ...
                                          conv_defs=conv_defs,partition_layer=partition_layer)
      ''''''
